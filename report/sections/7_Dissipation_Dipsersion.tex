...
\end{equation}
but now we extend the simulation to $T = 5\,\mathrm{s}$ in order to let phase and amplitude errors accumulate.
...
\[ \Delta t \text{ in }  1.5 {\times} 10^{-1},\, 10^{-1},\, 5 {\times} 10^{-2},\, 2 {\times} 10^{-2},\, 10^{-2},\, 5 \times 10^{-3},\, 2 \times 10^{-3},\, 10^{-3},\, 5 {\times} 10^{-4},\, 10^{-4},\, 5 {\times} 10^{-5} \]

\noindent For the two conditionally stable schemes (Forward Euler and explicit Newmark), the CFL condition is enforced and runs that violate it are ignored. The sweep is driven by the Python script \texttt{dissipation\_dispersion\_sweep.py}, which logs at every time step the discrete energy and the numerical solution at the domain centre.
...
\begin{itemize}
    \item \textbf{Crank--Nicolson} ($\theta = 0.5$) and \textbf{Newmark Average Acceleration} ($\beta = 0.25,\; \gamma = 0.5$) preserve the energy for all tested $\Delta t$, confirming their energy-conserving character.
...
\begin{itemize}
    ...
    \item \textbf{Crank--Nicolson} and \textbf{Newmark Average Acceleration}: the amplitude is perfectly preserved (no dissipation), but a phase shift develops for larger values of $\Delta t$. For example, at $\Delta t = 0.1$ a visible lag accumulates by $t \approx 3\,\mathrm{s}$, while for $\Delta t = 0.15$ the numerical wave is significantly out of phase by the end of the simulation. For sufficiently small $\Delta t$ the dispersion error becomes negligible.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{images/dissdisp_probe_vs_exact.png}

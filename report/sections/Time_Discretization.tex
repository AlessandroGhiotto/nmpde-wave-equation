...
To apply standard time-marching techniques, it is common practice to recast (9) as a system of first-order ordinary differential equations \cite{quarteroni2008numerical}. We introduce the discrete velocity vector $\mathbf{V}(t) = \dot{\mathbf{U}}(t)$, allowing us to rewrite the problem as two coupled equations:
\begin{equation}
\label{eq:first_order_system}
\begin{cases}
\dot{\mathbf{U}}(t) - \mathbf{V}(t) = \mathbf{0} \\
\mathbf{M}\dot{\mathbf{V}}(t) + \mathbf{A}\mathbf{U}(t) = \mathbf{F}(t)
\end{cases}
\end{equation}

Boundary conditions are naturally inherited by the velocity field; for a prescribed Dirichlet datum $\mathbf{U}(t) = \mathbf{g}(t)$ on the boundary nodes, the corresponding velocity constraint is strictly enforced as $\mathbf{V}(t) = \dot{\mathbf{g}}(t)$. This coupled first-order system forms the basis for the $\theta$-method family, while the original second-order form (9) is utilized directly by the Newmark-$\beta$ integration scheme.
...
\begin{equation}
\label{eq:theta_v}
\mathbf{M} \frac{\mathbf{V}^{n+1} - \mathbf{V}^n}{\Delta t} + \mathbf{A} (\theta \mathbf{U}^{n+1} + (1-\theta) \mathbf{U}^n) = \mathbf{F}_{\theta}^{n+1}
\end{equation}
...
(\mathbf{M} + \theta^2 \Delta t^2 \mathbf{A}) \mathbf{U}^{n+1} = \mathbf{M}(\mathbf{U}^n + \Delta t \mathbf{V}^n) - \theta(1-\theta)\Delta t^2 \mathbf{A} \mathbf{U}^n + \theta \Delta t^2 \mathbf{F}_{\theta}^{n+1}
\end{equation}
...
\begin{equation}
\mathbf{M}\mathbf{a}^{n+1} + \mathbf{A}\mathbf{U}^{n+1} = \mathbf{F}^{n+1}
\end{equation}
...
(\mathbf{M} + \beta \Delta t^2 \mathbf{A})\mathbf{a}^{n+1} = \mathbf{F}^{n+1} - \mathbf{A}\mathbf{U}^n - \Delta t \mathbf{A}\mathbf{V}^n - \Delta t^2 \left(\frac{1}{2} - \beta\right)\mathbf{A}\mathbf{a}^n
\end{equation}

...
\end{equation}
where $s$ denotes the spatial convergence order ($r{+}1$ in the $L^2$ norm, $r$ in the $H^1$ norm for polynomial degree~$r$) and $q$ is the temporal convergence order of the time-stepping scheme, see \cite{Bradji2014, Karaa2011}.
...
The convergence sweep explores the parameter grid reported in Table~\ref*{tab:conv_params}.

\begin{table}[htbp]
    \centering
    \caption{Parameter grid used in the convergence sweep.}
    ...
    \begin{tabular}{ll}
        ...
        Time step $\Delta t$ & $10^{-1}$, $5 \times 10^{-2}$, \ldots, $10^{-4}$ (10 values) \\
        \bottomrule
    \end{tabular}
\end{table}

\noindent The five schemes which are tested are reported in Table~\ref*{tab:conv_schemes}.

\begin{table}[htbp]
    \centering
    \caption{All the time discretizations schemes tested.}
    \label{tab:conv_schemes}
    \begin{tabular}{llcc}
        \toprule

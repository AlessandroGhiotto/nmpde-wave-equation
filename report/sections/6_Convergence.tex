...
\noindent For the two conditionally stable schemes (Forward Euler and explicit Newmark), the sweep script automatically filters out $(N_{\mathrm{el}}, \Delta t)$ combinations that violate the CFL condition, using a safety factor of $0.9$. Runs that produce relative errors above $10^5$ are flagged as diverged and excluded from the analysis. The total sweep comprises 490 solver runs.

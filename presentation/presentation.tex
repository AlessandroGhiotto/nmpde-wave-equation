...
\begin{frame}{Weak Formulation}
    Let $\mathcal{R}_\phi \in V_\phi$ be a lifting operator, then we decompose the solution as $u = \hat{u} + \mathcal{R}_\phi$.

    \vspace{0.3cm}
        \begin{block}{Variational Problem}
        Find $\hat{u}(t) \in V$ with $\hat{u}(0) = u_0 - \mathcal{R}_\phi(0)$ and $\partial_t \hat{u}(0) = v_0 - \partial_t \mathcal{R}_\phi(0)$, such that:
        \begin{equation*}
    ...
    \begin{itemize}
        ...
        \item \textbf{RHS:} $G(v) = \int_{\Omega} f v \, d\Omega - \int_{\Omega} \partial_{tt} \mathcal{R}_\phi \; v \, d\Omega - a(\mathcal{R}_\phi, v)$
    \end{itemize}
\end{frame}
...
\begin{frame}{Test Problem}
    ...
    u(\mathbf{x}, 0) = \sin(\pi x)\sin(\pi y) & \text{in } \Omega \\[6pt]
    \partial_t u(\mathbf{x}, 0) = 0 & \text{in } \Omega
    \end{cases}
    \end{equation*}
    \end{block}
...
\vspace{0.3cm}
    ...
    \begin{equation*}
    u_{\text{exact}}(\mathbf{x}, t) = \cos(\sqrt{2}\pi t) \sin(\pi x)\sin(\pi y)
    \end{equation*}
...
\begin{frame}{Spatial Convergence: Backward Euler ($\theta = 1$)}
    \centering
    \includegraphics[width=0.85\textwidth]{images/conv_theta_1_h_r1_2.png}
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Plateau Effect:} For $r=2$, the $L^2$ error is bounded by $C(h^3 + \Delta t)$, causing an early stall in convergence.
        \item The 1st-order temporal error dominates quickly as the mesh is refined ($h \to 0$).
    \end{itemize}
\end{frame}

\begin{frame}{Spatial Convergence: Crank-Nicolson ($\theta = 0.5$)}
    \centering
    \includegraphics[width=0.85\textwidth]{images/conv_theta_05_h_r1_2.png}
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Improved Accuracy:} The error is bounded by $C(h^s + \Delta t^2)$, allowing for a wider range of visible spatial rates.
        \item Spatial curves collapse onto each other for small $\Delta t$, indicating the pure spatial rate is correctly captured.
    \end{itemize}
\end{frame}

\begin{frame}{Temporal Convergence: Backward Euler ($\theta = 1$)}
    \centering
    \includegraphics[width=0.85\textwidth]{images/conv_theta_1_dt_r1_2.png} % Esempio BE
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Linear Rate:} Forward Euler ($\theta=0$) and Backward Euler ($\theta=1$) confirm the expected $\mathcal{O}(\Delta t)$ asymptotic behavior.
        \item \textbf{Spatial Plateau:} For small $\Delta t$, the error stops decreasing as it becomes dominated by the mesh size $h$.
    \end{itemize}
\end{frame}


\begin{frame}{Temporal Convergence: Cranck-Nicolson ($\theta = 0.5$)}
    \centering
    \includegraphics[width=0.85\textwidth]{images/conv_theta_05_dt_r1_2.png} % Esempio CN
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Quadratic Rate:} Crank-Nicolson and Newmark variants ($\gamma=0.5$) exhibit a much faster error reduction, following $\mathcal{O}(\Delta t^2)$.
        \item \textbf{Efficiency:} High-order temporal schemes allow for larger time steps while maintaining a precision comparable to the spatial discretization.
    \end{itemize}
\end{frame}
